...
def excel_dr(excel,table_name):
wb = xlrd.open_workbook(excel)  #打开excel文件
...
conn=mysqlclient.connect(host='localhost',user='root',passwd='××××××',db='db') #连接数据库
cursor=conn.cursor()  #创建游标

cursor.execute("create table table_name("+fo[0][0]+" varchar(100));")  #创建table命名table_name，添加第一个字段名
